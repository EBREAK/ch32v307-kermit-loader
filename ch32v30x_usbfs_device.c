...
__attribute__((aligned(4))) uint8_t HID_Report_Buffer[8];

/* Endpoint Buffer */
__attribute__((aligned(4))) uint8_t USBFS_EP0_Buf[64];
__attribute__((aligned(4))) uint8_t USBFS_EP1_Buf[64];
__attribute__((aligned(4))) uint8_t USBFS_EP4_Buf[64];
...
void USBFS_Device_Init(FunctionalState sta)
{
	if (sta) {
		...
		USBFSD->BASE_CTRL = USBFS_UC_DEV_PU_EN | USBFS_UC_INT_BUSY |
				    USBFS_UC_DMA_EN | USBFS_UC_LOW_SPEED;
	...
	if (intflag & USBFS_UIF_TRANSFER) {
		...
		case USBFS_UIS_TOKEN_IN:
			...
			case USBFS_UIS_TOKEN_IN | DEF_UEP0:
				...
				} else if ((USBFS_SetupReqType &
					    USB_REQ_TYP_MASK) ==
					   USB_REQ_TYP_CLASS) {
					switch (USBFS_SetupReqCode) {
					case HID_SET_REPORT:
						memcpy(&HID_Report_Buffer[0],
						       USBFS_EP0_Buf,
						       DEF_USBD_UEP0_SIZE);
						HID_Set_Report_Flag =
							SET_REPORT_WAIT_DEAL;
						USBFSD->UEP0_TX_CTRL =
							USBFS_UEP_T_TOG |
							USBFS_UEP_T_RES_NAK;
						break;
			...
			case (USBFS_UIS_TOKEN_IN | DEF_UEP4):
				...
				USBFSD->UEP4_TX_CTRL = (USBFSD->UEP4_TX_CTRL &
		...
		case USBFS_UIS_TOKEN_SETUP:
			...
			if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) ==
				...
				case USB_GET_DESCRIPTOR:
					switch ((uint8_t)(USBFS_SetupReqValue >>
							  8)) {
					...
					len = (USBFS_SetupReqLen >=
					       DEF_USBD_UEP0_SIZE) ?
						      DEF_USBD_UEP0_SIZE :
						      USBFS_SetupReqLen;
					memcpy(USBFS_EP0_Buf, pUSBFS_Descr,
					       len);
					pUSBFS_Descr += len;
					break;
				...
				case USB_CLEAR_FEATURE:
					if ((USBFS_SetupReqType &
						...
						if ((uint8_t)(USBFS_SetupReqValue &
							      0xFF) ==
			...
			} else if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) ==
				   USB_REQ_TYP_CLASS) {
				switch (USBFS_SetupReqCode) {
				case HID_SET_REPORT:
					break;
